...
# todo: add global func to convert output string -> int
    ...
    def __init__(self, data_set: np.array, theta: list = None, normalization: bool = False,
        ...
        if normalization:
            ...
            for i in range(c):
                ...
                if np.std(data_set[:, i]) != 0.0:  # features that are constant for all examples have a std of 0
                    self.stds.append(np.std(data_set[:, i]))
                    self.x[:, i] = self.x[:, i] / np.std(data_set[:, i])

                else:
                    self.stds.append(1)  # leave features with std=0 unscaled
        ...
        self._outputs = int(np.max(data_set[:, -1]))  # assuming outputs are labeled correctly
        self.y = np.zeros([self._outputs, 1])
        for output in data_set[:, -1]:  # asserting that output variations are sorted and differentiated by integers
            if int(output) != output:
                raise TypeError("Output values must be complete numbers only")

            else:
                outputvector = np.zeros([self._outputs, 1])
                if output != 0:  # in case all outputs are zero
                    outputvector[int(output)-1, 0] = 1  # important: minus one because index starts @ 0. none = all zero

                self.y = np.c_[self.y, outputvector]

        self.y = self.y[:, 1:]  # delete initial element from array creation
...
class Model(DataSet):
    ...
    @staticmethod
    def sigmoicfunc(z):  # sigmoid logistic equation
        return 1/(1+math.e**z)

    def _training_predictions(self):  # hypothesis func
        ...
        pred = self.sigmoicfunc(h)
        return pred  # returns vectors
        # return h.reshape(self.y.shape[0], 1)  # reshape

    def _costfunction(self, error):
        # todo: cost function log reg
        j = (1/self.m)*error  # returns a vector of predictions
        if self._regmode:
            regterm = np.sum(self.opt_theta**2)*self.lmbda
            j += regterm
        self.jlist.append(float(sum(sum(j))))
        return j
    ...
    def _build_model(self):
        thetas = self.theta
        for output in range(1, self._outputs+1):  # start from 1 to avoid output = 0
            self.jlist = []  # to reset for each output training
            self.theta = thetas  # to reset for each output training
            for i in range(self.iters):
                self._gradient_descent(output)
                self.iter_num = i + 1
                if i > 2:
    ...
    def predict(self, features: np.array, withbias: bool = False) -> np.array:   # pass features without bias unit
        # todo: if sigmoid above 0.5 then 1
    ...
    def _genereta_decisionboundary(self):
        pass


if __name__ == "__main__":
    andrewsdataset = load_data("ex2data2.txt")
